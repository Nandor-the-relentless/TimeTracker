...
export default function Dashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Load current active time entry
      const activeEntries = await TimeEntry.filter({
        user_id: currentUser.id,
        end_time: null
      });
      
      setCurrentEntry(activeEntries[0] || null);

      // Load today's hours
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      const todayEntries = await TimeEntry.filter({
        user_id: currentUser.id
      });

      const todayFiltered = todayEntries.filter(entry => {
        const entryDate = new Date(entry.start_time);
        return isToday(entryDate);
      });

      const todayTotal = todayFiltered.reduce((sum, entry) => {
      ...
      const weekFiltered = todayEntries.filter(entry => {
        const entryDate = new Date(entry.start_time);
        return entryDate >= weekStart && entryDate <= weekEnd;
      });

      const weekTotal = weekFiltered.reduce((sum, entry) => {
        return sum + (entry.duration_minutes || 0);
      }, 0);
      ...
      // Load recent entries
      const recent = await TimeEntry.filter({
        user_id: currentUser.id
      }, "-start_time", 5);
